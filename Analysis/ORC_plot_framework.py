...
def csv_to_dict_v6(csv_path):
    """
    Convert a CSV file with columns 'title' and 'pages' to a dictionary.
    ...
    with open(csv_path, 'r') as csvfile:
        ...
        for row in reader:
            # Identify the last non-numeric element to determine the title boundary
            title_boundary = max([i for i, item in enumerate(row) if not item.isdigit()])
            
            title = ",".join(row[:title_boundary + 1])
            pages = list(map(int, row[title_boundary + 1:]))
...
def save_data_points_to_csv(data_points, csv_filename):
    """
...
def generate_page_dict(folder_path):
...
file_dic = generate_page_dict(folder_path)
...
#%%
#load csv
page_directory = csv_to_dict_v6("../Data/Pre_processing/page_directory.csv")
process_pdfs(folder_path, page_directory)
