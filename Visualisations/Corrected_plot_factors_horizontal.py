...
def plot_effect_magnitude(effect_magnitude_data, ax):
    ...
    symbols = {'+': 0, '-': 1, '±': 2}
    y_positions = {variable: i * 3 for i, variable in enumerate(variables)}  # Increase spacing between y-ticks
    color_mapping = {'+': 'g', '-': 'r', '±': 'b'}
...
def main():
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 7), gridspec_kw={'width_ratios': [1, 0.8], 'wspace': 0.1})
    plot_effect_cascade_success(effect_cascade_data, size_mapping_legend, ax1)
    plot_effect_magnitude(effect_magnitude_data, ax2)
    
    # Draw vertical line between plots
    line = plt.Line2D((0.55, 0.55), (0.1, 0.9), color='black', linewidth=2, transform=fig.transFigure, figure=fig)
    fig.add_artist(line)
    
    # Add (a) and (b) labels
    ax1.text(0.5, 1.1, '(a)', transform=ax1.transAxes, fontsize=16, fontweight='bold', va='top', ha='right')
    ax2.text(0.5, 1.1, '(b)', transform=ax2.transAxes, fontsize=16, fontweight='bold', va='top', ha='right')
    
    plt.tight_layout()
    plt.savefig("../Figures/Combined_plot_final.png", dpi=300, bbox_inches="tight")
    plt.show()
